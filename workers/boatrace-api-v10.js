...
async function handleRequest(request, env) {
  ...
  try {
    ...
    if (path === '/api/prize-ranking') {
      // アクセス時点の最新を反映するため、まず公式(grandprix.jp)から取得を試みる。
      // 失敗時のみ DB(seed) にフォールバックする。
      // ?source=db でDB強制、?refresh=1 は互換のため受容
      const forceDb = url.searchParams.get('source') === 'db';

      if (!forceDb) {
        try {
          // /2026/rtg/ に最新の獲得賞金ランキング(全選手・日付付き)がある
          const html = await fetchHtml('https://www.boatrace-grandprix.jp/2026/rtg/');
          const parsed = HTMLParser.parsePrizeRanking(html);
          if (parsed.length > 0) {
            // フロント互換の形へ正規化（racerId / 数値prizeMoney / class）
            const rankings = parsed.map((r) => ({
              rank: r.rank,
              racerId: String(r.registrationNumber),
              name: (r.name || '').replace(/\u3000/g, ' ').replace(/\s+/g, ' ').trim(),
              branch: r.branch || null,
              class: r.rankClass || null,
              prizeMoney: parseInt(String(r.prizeMoney).replace(/[^\d]/g, ''), 10) || 0,
            }));
            return jsonResponse(
              { rankings, source: 'boatrace-grandprix.jp', updatedAt: new Date().toISOString() },
              { 'Cache-Control': 'public, max-age=1800, s-maxage=1800' }
            );
          }
        } catch (e) {
          console.error('公式賞金ランキング取得エラー(→DBへフォールバック):', e);
        }
      }
      ...
      return jsonResponse({ error: '賞金ランキングの取得に失敗しました', rankings: [] }, {}, 500);
    }
