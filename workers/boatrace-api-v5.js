...
class HTMLParser {
   ...
   * 各行: 日付(MM/DD-MM/DD) / 会場(img alt) / グレードclass / レース名(hd付きリンク)
   * 戻り値: [{ raceId, raceName, venueName, venueCode, hd, grade, startDate, endDate }]
   */
  static parseGradeSchedule(html) {
    try {
      const races = [];
      ...
      while ((match = rowRegex.exec(html)) !== null) {
        ...
        // レース名と hd（終了日の年月日 YYYYMMDD が含まれる）
        const nm = rowHtml.match(/is-alignL"><a[^>]*hd=(\d{8})[^>]*>([^<]+)<\/a>/);
        if (!nm) continue;
        const hd = nm[1];
        const raceName = nm[2].trim();

        // グレード（is-SGa / is-G1b など）
        const gm = rowHtml.match(/is-(SG|G1|G2|G3)[ab]?/);
        const grade = gm ? gm[1] : '一般';

        // 年の決定: hd(終了日)を基準に、開始月>終了月なら年跨ぎで開始は前年
        const endYear = parseInt(hd.slice(0, 4), 10);
        ...
        // race_id: 場コード + 終了日hd（選手スケジュールと共通の一意キー）
        const raceId = venueCode && hd ? `${venueCode}_${hd}` : `${startDate}_${venueName || 'unknown'}`;

        if (venueName || raceName) {
          races.push({ raceId, raceName, venueName, venueCode, hd, grade, startDate, endDate });
        }
      }
...
async function fetchGradeScheduleFromSite(hcd) {
  const response = await fetch(
  ...
  if (!response.ok) {
    throw new Error(`グレードスケジュール取得失敗(hcd=${hcd}): ${response.status}`);
  }
  const html = await response.text();
  return HTMLParser.parseGradeSchedule(html);
}
...
async function getGradeRacesFromDB(env, grades) {
          ...
          if (races.length === 0 || forceRefresh) {
            try {
              ...
              const [sgList, g1List] = await Promise.all([
                fetchGradeScheduleFromSite('01'),
                fetchGradeScheduleFromSite('02'),
              ]);
